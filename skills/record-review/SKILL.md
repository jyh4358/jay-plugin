...
- **방치 ongoing**: 노트별로 처분을 묻는다 — 계속 진행 / 완료 마감(`status: done` + `date_end` + 진행 이력 한 줄) / 중단 기록(done 처리하되 진행 이력에 중단 사유). 상태를 추정으로 일괄 변경하지 않는다. **계속 진행**을 선택하면 `## 진행 이력`에 "YYYY-MM-DD 점검 — 계속 진행" 한 줄을 남기고 `updated`를 갱신한다 — 다음 점검에서 같은 노트를 또 묻지 않도록.
